...
interface UpstoxLoginDialogProps {
  open: boolean;
...
export const UpstoxLoginDialog = ({
  open,
  onOpenChange,
  brokerId,
  onLoginSuccess,
}: UpstoxLoginDialogProps) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const brokerName = brokerId?.toLowerCase() || "upstox";
      ...
      const response = await apiFetch(
        `http://localhost:3000/api/brokers/login`,
        {
          ...
          body: JSON.stringify({
            ...
            redirectURI: formData.redirectUri || "http://localhost:3000/callback",
      ...
      <DialogContent className="sm:max-w-[500px]">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            <span>Connect to UPSTOX</span>
          </DialogTitle>
          <DialogDescription>
            Enter your UPSTOX credentials to connect your account
          </DialogDescription>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <Input
              ...
              disabled={isLoading}
              className={errors.redirectUri ? "border-destructive" : ""}
            />
            {errors.redirectUri && (
              <p className="text-sm text-destructive">{errors.redirectUri}</p>
            )}
            <p className="text-xs text-muted-foreground">
              The redirect URI registered with your UPSTOX app
            </p>
          </div>
